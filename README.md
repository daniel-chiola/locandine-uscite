...
Il sito non ha password: **chiunque abbia il link può usarlo**, e il codice è visibile su GitHub perché il repository è pubblico (è la condizione per avere GitHub Pages gratis).

Non è un problema di sicurezza — l'app non contiene dati, non salva niente online — ma tienilo presente prima di considerarla "privata".

---

## Aggiornare l'app

Dopo aver modificato `index.html` e ricaricato i file su GitHub, **cambia il numero di versione** in `sw.js`:

```js
const CACHE = 'locandine-v3';   // → 'locandine-v4'
```

Senza questo passaggio i telefoni che hanno già installato l'app continuano a vedere la **versione vecchia**, presa dalla cache.
...
| File | A cosa serve |
|---|---|
| `index.html` | Tutta l'app: interfaccia + motore di disegno |
| `sw.js` | Service worker: fa funzionare l'app offline |
| `manifest.webmanifest` | Nome, icone e colori dell'app installata |
| `icon-192.png` `icon-512.png` | Icone sulla schermata Home |

**Un solo file, zero dipendenze esterne.** Le locandine sono disegnate con `<canvas>` puro: nessuna libreria da scaricare, nessun font remoto, nessuna chiamata a servizi esterni. È il motivo per cui funziona offline e continuerà a funzionare anche fra anni, senza manutenzione.

L'unica libreria inclusa è **qrcode-generator** (MIT), incorporata nel file per generare il QR senza connessione.
